...
-- ----------------------------------------------------------------------------
-- Gerard Lemson (glemson1@jhu.edu): this version is changed so that an 
-- automated transofrmation script creates valid MS SQL T-SQL.
-- main changes (indicated with MSSQL_CHANGE): 
--  . COMMENT statements commented out (no equivalent known to us at this time)
--  . various varchar(25) columns changed to varchar(63) so that foreign key to 
--    type_of_cancer([TYPE_OF_CANCER_ID]) is ok-ed by MS SQL. 
--  . 
-- ----------------------------------------------------------------------------
--
-- Database: `cgds`
...
-- order the CREATE_TABLE statements so that refererenced
...
CREATE TABLE `cancer_study` (
  ...
  `TYPE_OF_CANCER_ID` varchar(63) NOT NULL, -- MSSQL_CHANGE changed from varchar(25)
  `NAME` varchar(255) NOT NULL,
...
CREATE TABLE `sample` (
  ...
  `TYPE_OF_CANCER_ID` varchar(63) NOT NULL, -- MSSQL_CHANGE changed form varchar(25)
  PRIMARY KEY (`INTERNAL_ID`),
...
CREATE TABLE `uniprot_id_mapping` (
  ...
  `ENTREZ_GENE_ID` int(11) NOT NULL, -- MSSQL_CHANGE made not null, otherwise can not be part of primary key in MS SQL
...
CREATE TABLE `mutation_event` (
  ...
  `MUTATION_TYPE` varchar(255), -- MSSQL_CHANGE COMMENT 'e.g. Missense, Nonsence, etc.',
  `FUNCTIONAL_IMPACT_SCORE` varchar(50), -- MSSQL_CHANGE COMMENT 'Result from OMA/XVAR.',
  `FIS_VALUE` float,
  ...
  FOREIGN KEY (`ENTREZ_GENE_ID`) REFERENCES `gene` (`ENTREZ_GENE_ID`)
); -- MSSQL_CHANGE  COMMENT='Mutation Data';
...
CREATE TABLE `mutation` (
  ...
  `MUTATION_STATUS` varchar(25), -- MSSQL_CHANGE COMMENT 'Germline, Somatic or LOH.',
  ...
  FOREIGN KEY (`GENETIC_PROFILE_ID`) REFERENCES `genetic_profile` (`GENETIC_PROFILE_ID`) ON DELETE CASCADE,
  FOREIGN KEY (`SAMPLE_ID`) REFERENCES `sample` (`INTERNAL_ID`) ON DELETE CASCADE
); -- MSSQL_CHANGE COMMENT='Mutation Data Details';
...
CREATE TABLE `clinical_attribute_meta` (
  ...
  `DATATYPE` varchar(255) NOT NULL, -- MSSQL_CHANGE COMMENT 'NUMBER, BOOLEAN, or STRING',
...
CREATE TABLE `sanger_cancer_census` (
  ...
  FOREIGN KEY (`ENTREZ_GENE_ID`) REFERENCES `gene` (`ENTREZ_GENE_ID`)
); -- MSSQL_CHANGE COMMENT='Sanger Cancer Gene Census';
...
CREATE TABLE `cna_event` (
  ...
  `ENTREZ_GENE_ID` int(11) NOT NULL,
  `ALTERATION` smallint NOT NULL, -- MSSQL_CHANGE tinyint -> smallint. somehow tinyint gives problems in MS SQL when copying data form the MySQL DB.
